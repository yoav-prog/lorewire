...
Status: Phase 1 implemented 2026-06-17 (see Implementation status below); awaiting credentials + Phase 0 review
Owner: Yoav
...
Phase 1 (YouTube) is built and tested on the `feat/reels-feed` branch. Live
publishing is gated only on credentials and the Phase 0 review applications, not
on code.

### Done

- **Phase 0 code artifacts** (shipped earlier): public `/privacy`, `/terms`, the Meta data-deletion callback + status page.
- **Phase 1, end to end:**
  - Token cipher (AES-256-GCM, key-fingerprint rotation) and the `social_accounts`, `youtube_publishes`, `oauth_flows` tables (TEXT ids + base64 token envelopes, so it runs on both SQLite and Postgres). §6, §8, §10
  - `redact()` log guard, audio-clearance gate (F9), exponential-backoff retry. §3.F9, §11
  - YouTube OAuth connect + callback: auth code + PKCE + session-bound single-use `state` + TTL, tokens sealed at rest. §8
  - Settings to Social accounts panel: Connect / Reconnect / Disconnect / status. §12
  - Publish engine: token refresh + reseal, streamed two-step resumable `videos.insert`. §7.1
  - Publish route `POST /api/social/youtube/publish` (`maxDuration=300`): readiness + connection guards, idempotency, audio gate, story-to-metadata mapper + validation, ledger writes, observability namespaces. §5, §9, §11
  - "Publish to YouTube" button on the short editor with a confirm() quota gate. §9
  - Tests: unit (cipher, redact, gate, retry, OAuth PKCE/state, metadata) + integration (query layers, token refresh, route orchestration). Full suite green. §13

### Left

- **Phase 0 owner tasks (not code):** provision the business Google + Meta accounts, submit the four review applications, set env (`GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET`, `LOREWIRE_TOKEN_KEY`, `NEXT_PUBLIC_SITE_ORIGIN`), and register the Google redirect URI `<origin>/api/social/oauth/youtube/callback`. §8, §15
- **Phase 1 hardening deferred to Phase 2** (noted in code): single-flight token refresh via advisory lock (§N4), race-proof idempotency via the `publish_jobs` partial index, processing-to-live status polling, and a styled confirmation modal instead of `window.confirm` (F3).
- **Phase 2:** Instagram + Facebook Reels, the `publish_requests` / `publish_jobs` queue, signed GCS URLs, customize-per-platform UI, metrics. §15
- **Phase 3:** TikTok (sandbox-only drafts until the audit clears). §15
- **Phase 4/5:** Cloud Run worker (only if forced) and polish. §15
